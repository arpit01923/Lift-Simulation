...
function submitHandler(e) {
  ...
  const floorError = document.querySelector("#floorError");
  const totalLifts = document.querySelector("#lifts");
  const liftError = document.querySelector("#liftError");
  console.log(totalFloors, totalLifts);

  if (totalFloors.value > 10) {
    floorError.innerText = "Max floor limit is 10";
    return;
  } else if (totalLifts.value > 5) {
    liftError.innerText = "Max lift limit is 5";
    return;
  } else {
    floorError.innerText = "";
    liftError.innerText = "";
